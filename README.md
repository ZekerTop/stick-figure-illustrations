...
这是一个适合放在 Codex 里的创作辅助 skill：把文章、公众号封面、文档、教程、PPT、社媒内容、开源项目说明和产品状态，转成以**有情绪的中性火柴人**为默认主体的草图式解释插画。
...
- 正常人物默认只有两只手、两只脚；除非明确表达很忙、多任务或全能，否则不要多手多脚
- 用户未指定时默认 `16:9` 画幅
- 白底或透明背景
- 黑色线稿
- 一个克制点缀色；用户不需要手动填写
...
| 公众号封面 | 手绘头图、标题概念图、强场景封面 | `2.35:1` / 用户指定比例 |
...
- 需要大量文字排版的商业海报、品牌 KV 或纯排版封面终稿

如果你需要的是“设计师级成品海报”，这个 skill 更适合作为前期概念草图和 shot list 工具；如果你要的是公众号手绘概念封面，可以使用 `wechat-cover` preset。
...
| `wechat-cover` | 公众号封面、头图、宽封面 | 1-3 | 默认 `2.35:1` | 大物件 + 人物动作 + 变化结果 |
...
```text
Use $stick-figure-illustrations 给这个标题生成一张公众号封面图，大小 2.35:1，走 wechat-cover。

标题：代码写得越来越快，我们却越来越说不清项目过程，于是我做了个复盘 Skill

不要做海报模板，也不要做居中人物 + 漂浮卡片 + 时间线 + 勾选文档。
画一个强主场景：一个火柴人站在巨大的终端窗口前，双手拉开幕布。
幕布前面只有一行简单结果，幕布后面露出 Git 提交、AI 会话、改方向、交付记录被一根蓝色线串成清楚路径，最后进入一本“项目复盘”笔记。
画面主体是“拉开后看到背后过程”的瞬间，文字只保留手写短标题和 2-3 个物件标签。
```

公众号封面必须是自然手绘感的栅格图。不要用 SVG、HTML 截图、流程图或线框图当最终封面。
