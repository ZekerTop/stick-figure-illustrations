...
把这些提示词复制到 Codex 里，再替换占位内容。示例只保留用户需要提供的内容和目标；画幅、视觉风格、火柴人情绪、表达性短标注和质量检查由 skill 默认处理。
...
默认就是有表现力的观点图，不需要在提示词里额外写“有表现力”。除非你明确要求无文字，否则会自动加入短标题、物件标签、状态词或 punchline，让画面更好懂也更有趣。
...
公众号封面默认应该是图像生成得到的自然手绘栅格图。不要用 SVG、HTML 截图、流程图或线框图当最终封面。
